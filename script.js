document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- Funções Auxiliares ---
    function populateSuggestedScenarios() { if (typeof SCENARIOS === 'undefined') { console.error("Objeto de cenários não encontrado!"); return; } suggestedScenariosList.innerHTML = ''; Object.keys(SCENARIOS).forEach(categoryName => { const categoryGroup = document.createElement('div'); categoryGroup.className = 'category-group collapsible collapsed'; categoryGroup.dataset.category = categoryName; const categoryHeader = document.createElement('p'); categoryHeader.className = 'category-header'; categoryHeader.innerHTML = `${categoryName} <span class="toggle-icon">▼</span>`; const scenarioItems = document.createElement('ul'); scenarioItems.className = 'scenario-items'; Object.keys(SCENARIOS[categoryName]).forEach(scenarioName => { const li = document.createElement('li'); li.textContent = scenarioName; li.dataset.categoryName = categoryName; li.dataset.scenarioName = scenarioName; scenarioItems.appendChild(li); }); categoryGroup.appendChild(categoryHeader); categoryGroup.appendChild(scenarioItems); suggestedScenariosList.appendChild(categoryGroup); }); }
